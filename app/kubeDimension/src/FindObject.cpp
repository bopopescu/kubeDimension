...
FindObject::FindObject(bool keepImagesInRAM, bool distributedSearch, int workersCopy, int workersShards, QObject * parent) :
	QObject(parent),
	...
	distributedSearch_(distributedSearch),
	workersCopy_(workersCopy),
	workersShards_(workersShards)
{
...
bool FindObject::loadSession(const QString & path)
{
	if(QFile::exists(path) && !path.isEmpty() && QFileInfo(path).suffix().compare("bin") == 0)
	{
		...
		file.close();
		UDEBUG("Closing the current object!");

		if(!Settings::getGeneral_invertedSearch())
		{
			// this will fill objectsDescriptors_ matrix
			updateVocabulary();
			UDEBUG("this will fill objectsDescriptors_ matrix!");
		}
		sessionModified_ = false;
		return true;
	}
...
const ObjSignature * FindObject::addObject(const QString & filePath)
{
	if(!filePath.isNull())
	{
		...
		if(!img.empty())
		{
			...
			if(s)
			{
				UDEBUG("Added object %d (%s)", s->id(), filePath.toStdString().c_str());
				return s;
			}
		}
		else
...
protected:
	virtual void run()
	{
		...
		UDEBUG("%d descriptors extracted from object %d (in %d ms)", descriptors_.rows, objectId_, time.elapsed());
	}
private:
	Feature2D * detector_;
	Feature2D * extractor_;
...
void FindObject::updateObjects(const QList<int> & ids)
{
	...
	if(objectsList.size())
	{
		...
		if(objectsList.size())
		{
			UDEBUG("Features extraction from %d objects...", objectsList.size());
			for(int i=0; i<objectsList.size(); i+=threadCounts)
			{
				...
				for(int j=0; j<threads.size(); ++j)
				{
					...
					//if(!keepImagesInRAM_)
					//{	
						UDEBUG("Remove Object #%d from Memory", objects_.value(id), time.elapsed());					
						objects_.value(id)->removeImage();
					//}
				}
			}
			UDEBUG("Features extraction from %d objects... done! (%d ms)", objectsList.size(), time.elapsed());
		}
	}
	else
	{
...
void FindObject::updateVocabulary(const QList<int> & ids)
{
	...
	if(count)
	{
		...
		else
		{
			...
			if(incremental)
			{
				UDEBUG("Creating incremental vocabulary...");
			}
			else if(Settings::getGeneral_vocabularyFixed())
			{
				UDEBUG("Updating vocabulary correspondences only (vocabulary is fixed)...");
			}
			else
			{
				UDEBUG("Creating vocabulary...");
			}
			QTime localTime;
			localTime.start();
			...
			for(int i=0; i<objectsList.size(); ++i)
			{
				...
				UDEBUG("Object %d, %d words from %d descriptors (%d words, %d ms) %s",
						objectsList[i]->id(),
			...
			if(incremental)
			{
				UDEBUG("Creating incremental vocabulary... done! size=%d (%d ms)", vocabulary_->size(), time.elapsed());
			}
			else if(Settings::getGeneral_vocabularyFixed())
			{
				UDEBUG("Updating vocabulary correspondences only (vocabulary is fixed)... done! size=%d (%d ms)", vocabulary_->size(), time.elapsed());
			}
			else
			{
				UDEBUG("Creating vocabulary... done! size=%d (%d ms)", vocabulary_->size(), time.elapsed());
			}
		}
	}
}
	...
	virtual void run()
	{
		...
		if((int)mpts_1.size() >= Settings::getHomography_minimumInliers())
		{
			if(Settings::getHomography_opticalFlow())
			{
				...
				//std::vector<cv::Mat> imageA_, imageB_;

				// precompute image pyramid using cv::buildOpticalFlowPyramid
				// cv::buildOpticalFlowPyramid(imageA_, imageB_, cv::Size(Settings::getHomography_opticalFlowWinSize(), Settings::getHomography_opticalFlowWinSize()), Settings::getHomography_opticalFlowMaxLevel(), true);
		    ...
		    try {
				...
				// Starting from approximately 2.4.5 release cv::findHomography can return empty matrix in some cases. 
		    ...
		    {
				UDEBUG("Inliers Object MPT_1 %d", (int)mpts_1.size());
				UDEBUG("Inliers Object MPT_2 %d", (int)mpts_2.size());
		        UERROR("Error findHomography: %s ", e.what());
		...
		//UINFO("Homography Object %d time=%d ms", code_, time.elapsed());
	}
private:
...
void FindObject::detect(const cv::Mat & image)
{
	...
	{
		UDEBUG("(%s) %d objects detected! (%d ms)",
	...
	{
		UDEBUG("(%s) Object %d detected! (%d ms)",
				QTime::currentTime().toString("HH:mm:ss.zzz").toStdString().c_str(),
	...
	{
		UDEBUG("(%s) No objects detected. (%d ms)",
				QTime::currentTime().toString("HH:mm:ss.zzz").toStdString().c_str(),
				time.elapsed());
	}
}
...
bool FindObject::detect(const cv::Mat & image, find_object::DetectionInfo & info)
{
	...
	if(!image.empty())
	{
		...
		if(grayscaleImg.cols == 0)
		{
			UDEBUG("No cols detected in the scene!?!");
			return false;
		} else {
			UDEBUG("cols detected in the scene: (%d)", grayscaleImg.cols);			
		}

        cv::Size s = grayscaleImg.size();
        if(s.height < 100 && s.width < 100) // define in the settings
        {        
			UDEBUG("Wrong dimension for the scene; width (%d) / height (%d)", s.width, s.height);
			return false;
		} else {
			UDEBUG("Dimension for the scene; width (%d) / height (%d)", s.width, s.height);
		}
