...
class FINDOBJECT_EXP Settings
{
	...
	PARAMETER(Camera, 2imageWidth, int, 320, "Image width (0 means default width from camera).");
	PARAMETER(Camera, 3imageHeight, int, 240, "Image height (0 means default height from camera).");
	PARAMETER(Camera, 4imageRate, double, 0.0, "Image rate in Hz (0 Hz means as fast as possible)."); // Hz
	...
	PARAMETER(Camera, 6useTcpCamera, bool, true, "Use TCP/IP input camera.");
	...
	PARAMETER(Camera, 9queueSize, int, 0, "Maximum images buffered from TCP. If 0, all images are buffered.");

	//List format : [Index:item0;item1;item3;...]

	PARAMETER_COND(Feature2D, 1Detector, QString, FINDOBJECT_NONFREE, "7:Dense;Fast;GFTT;MSER;ORB;SIFT;Star;SURF;BRISK;AGAST;KAZE;AKAZE" , "5:Dense;Fast;GFTT;MSER;ORB;SIFT;Star;SURF;BRISK;AGAST;KAZE;AKAZE", "Keypoint detector.");
	PARAMETER_COND(Feature2D, 2Descriptor, QString, FINDOBJECT_NONFREE, "3:Brief;ORB;SIFT;SURF;BRISK;FREAK;KAZE;AKAZE;LUCID;LATCH;DAISY", "2:Brief;ORB;SIFT;SURF;BRISK;FREAK;KAZE;AKAZE;LUCID;LATCH;DAISY", "Keypoint descriptor.");
	...
	// Need to be implement as it will improve the accuaracy of SIFT matching
	PARAMETER(Feature2D, 10RootSift, bool, false, "(Rootsift) Stable local invariant feature extraction method in comparison to SIFT.");
	...
	PARAMETER(General, proxyPort, int, 1986, "Port to the proxy distributing incoming scene to be recognize through specific recusive trees.")
	PARAMETER(General, distributedSearchSurveyorTimeout, int, 2000, "To guarantee timely delivery of results up the tree, the survey’s timeout has to be smaller going down the tree.");
	PARAMETER(General, distributedSearchRespondentsTimeout, int, 1000, "Recursive Tree Sub-services response timeout.");
	PARAMETER(General, distributedSearchAsyncMining, bool, false, "Send to an asynchornus queue for legacy mining if enough insights about the quality of the scene.");
