 ...
 */
RestHandler::RestHandler(HttpServerRequest *request,
 ...
 * - When the End event is triggered the buffer is passed to the onEnd method 
 */
void RestHandler::onData(const QByteArray &chunk)
{
    buffer += chunk;
}

void RestHandler::onError(const std::ostringstream & error, Json::Value & logs)
{
    Json::FastWriter fastWriter;
        ...
        if(!isFile) {
            ...
            if (fileData.size() <= 2048) {
                ...
                else 
                {
                    ...
                    cv::Mat sceneMat = scene.getMat();
                    if (sceneMat.channels() > 1)
                        cvtColor(scene, sceneMat, COLOR_BGR2GRAY);

                    Mat scene_32;
                    if ( sceneMat.depth() == CV_32F )
                        scene_32 = img;
                    else if ( sceneMat.depth() == CV_8U )
                        sceneMat.convertTo(scene_32, CV_32F, 1.0 / 255.0, 0);
                    else if ( sceneMat.depth() == CV_16U )
                        sceneMat.convertTo(scene_32, CV_32F, 1.0 / 65535.0, 0);

                    CV_Assert( ! scene_32.empty() );

                    // Debugging information about channels and depth
                    detect << "Number of channels: " << scene_32.channels() << ", (" << time.elapsed() << " ms)";            
                    logs["Info"].append(detect.str());
                    detect.str("");

                    detect << "Scene depth: " << scene_32.depth() << ", (" << time.elapsed() << " ms)";            
                    logs["Info"].append(detect.str());
                    detect.str("");

                    detect << "Number of channels: " << scene_32.type() << ", (" << time.elapsed() << " ms)";            
                    logs["Info"].append(detect.str());
                    detect.str("");
                    ...
                    */
                    ...
                    try {
                        sceneHeatmap(scene_32, fileNamePrefix);
                    } catch (exception& e) {
                    ...
                    if(!scene_32.data) {
                        ...
                        error << " Unable to decode the picture received, there is no data, (" << time.elapsed() << " ms)";
                        this->onError(error, logs);
                    ...
                    if (scene_32.cols == 0) {
                        ...
                        error << " conversion problem of the matrix, 0 cols detected, (" << time.elapsed() << " ms)";
                        this->onError(error, logs);
