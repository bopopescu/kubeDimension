...
void TcpServer::readReceivedData()
{
	...
	if(serviceType == kAddObject)
	{
		...
		UINFO("[TCP-SERVER] TCP service: Add %d \"%s\"", id, fileName.toStdString().c_str());
	...
	else if(serviceType == kRemoveObject)
	{
		...
		UINFO("[TCP-SERVER] TCP service: Remove %d", id);
	...
	else if(serviceType == kDetectObject)
	{
		...
		if(image.empty()) {
			UERROR("[TCP-SERVER] No image provided");
			ok = false;
		} else {
			UINFO("[TCP-SERVER] TCP service: compare a scene: %d ", imageSize);
			Q_EMIT detectObject(image);
		}
	} else {
		UERROR("[TCP-SERVER] Unkown Service");
		ok = false;		
	}
