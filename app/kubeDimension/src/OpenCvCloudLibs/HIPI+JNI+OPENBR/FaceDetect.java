...
public class FaceDetect extends Configured implements Tool {
 ...
 public static class FaceDetectMapper extends Mapper<ImageHeader, BytesWritable, Text, Text> {
	...
	String filePath2 = "~/compare.jpg";
	FaceDetectBR comparetool = new FaceDetectBR();
	double result = 0;
	comparetool.initopenBR();
	result = comparetool.compareResult(localpath.toString(),filePath2);
	comparetool.finishopenBR();
        context.write(new Text("compare.jpg"), new Text(hashval + ext + "," + Double.toString(result)));
      } // If (value != null...
